...
export function useGetCustomer() {
  const { data, isLoading, error, isValidating, mutate } = useSWR(API_BASE_URL + '/api/withdraw', fetcher, {
  ...
  const updateWithdraw = async (data) => {
    mutate(API_BASE_URL + `/api/withdraw`, data, false);
  };

  const memoizedValue = useMemo(
    () => ({
      customers: data?.data || [], // Ensure it always returns an array
...
export function useWithdraw(id) {
  const { data, isLoading, error, isValidating, mutate } = useSWR(API_BASE_URL + `/api/withdraw/${id}`, fetcherPut, {
    revalidateIfStale: false,
    revalidateOnFocus: false,
    revalidateOnReconnect: false
  });
  ...
  const memoizedValue = useMemo(
    () => ({
      withdraw: data?.data || [], // Ensure it always returns an array
