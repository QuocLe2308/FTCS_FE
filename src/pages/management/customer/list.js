...
import { fetcherPut } from 'utils/axios';
...
const CustomerListPage = () => {
  ...
  const { customersLoading, customers: lists, updateWithdraw } = useGetCustomer();
  ...
  const handleUpdateWithdraw = async (id,status = 'APPROVED') => {
    const res = await fetcherPut([
      API_BASE_URL + `/api/withdraw/${id}`,
      {
        status
      }
    ]);
        ...
        cell: ({ row, getValue }) => (
          <Stack direction="row" spacing={1.5} alignItems="center">
            <Stack spacing={0}>
              ...
              <Typography color="text.secondary">{row.original.bankAccountNumber}</Typography>
            </Stack>
          </Stack>
        )
      },
            ...
            <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
              <Tooltip title="Accept">
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleUpdateWithdraw(row.original.withdrawId, 'APPROVED');
                  }}
                >
                  <CheckOutlined />
                </IconButton>
              ...
              <Tooltip title="Deny">
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleUpdateWithdraw(row.original.withdrawId, 'REJECTED');
                  }}
                >
                  <CloseOutlined />
                </IconButton>
